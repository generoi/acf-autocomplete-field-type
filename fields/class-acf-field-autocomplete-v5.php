    ...
    class acf_field_autocomplete_field extends acf_field
    {
        ...
        */
        public function autocomplete_ajax_callback()
        {
            ...
            SELECT DISTINCT postmeta2.meta_value FROM $wpdb->postmeta as postmeta1, $wpdb->postmeta as postmeta2 
            WHERE postmeta1.meta_value = '%s' AND 
                  postmeta1.meta_key = CONCAT( '_', postmeta2.meta_key ) AND 
                  postmeta2.meta_value LIKE %s",
                  $_REQUEST['field_key'],
                  '%' . $wpdb->esc_like($_REQUEST['request']) . '%'
            ));
